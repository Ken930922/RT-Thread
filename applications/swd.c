...
//    uint8_t ack;
    ...
    ack1 = SWD_Transfer(req, &val);

    (ack1 == DAP_TRANSFER_OK) ? (err = 0) : (err = 1);
    return err;
}
...
static uint8_t SWJ_WriteMem8(uint32_t addr, uint8_t val)
{
    ...
    uint8_t err;
		...
		if (SWJ_WriteAP(AP_CSW, CSW_VALUE | CSW_SIZE8)) 
		{
        return 1;
    }
    tmp = val << ((addr & 0x03) << 3);
...
static uint8_t SWJ_WriteBlock(uint32_t addr, uint8_t *buf, uint32_t len)
{
    ...
    if( SWJ_WriteAP(AP_CSW, CSW_VALUE | CSW_SIZE32))
    {
        SWD_TRACE("AP_CSW, CSW_VALUE | CSW_SIZE32 failed\r\n");
				return 1;
    }
		if( SWJ_WriteAP(AP_TAR, addr))
    {
        return 1;
    }
...
uint8_t swd_write_debug_state(DEBUG_STATE *state)
{
    ...
    // check status
		...
		if( SWJ_ReadDP(DP_CTRL_STAT, &status))
		{
			return 1;
		}
...
uint8_t SWJ_InitDebug(void)
{
    ...
    if (SWJ_WriteDP(DP_CTRL_STAT, CSYSPWRUPREQ | CDBGPWRUPREQ | TRNNORMAL | MASKLANE)) 
		{
        return 1;
    }
